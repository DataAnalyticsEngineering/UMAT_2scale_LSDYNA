...
for file_name in sorted(glob.glob("*.txt")):
    ...
    # arr = arr[arr[:, 0] <= 25]
    if '1.6' in file_name:
        h1, = plt.plot(arr[:, 0], arr[:, 1], label=file_name, ls='dashdot', color='green')
    else:
        h2, = plt.plot(arr[:, 0], arr[:, 1], label=file_name, ls='--', color='blue')
...
arr2[:, 0] -= arr2[0, 0]
# arr2[:, 0] += arr[:, 0][idx]
h3, = plt.plot(arr2[:, 0], arr2[:, 1], label='Simulation', marker="*", markersize=5, color='black')
...
plt.legend(labels=['1.6 kW', '1.8 kW', 'Simulation'], handles=[h1, h2, h3])
plt.grid('on')
# plt.legend()
plt.tight_layout()
plt.savefig('thermal_cycles_v01.pdf', dpi=300)
plt.savefig('thermal_cycles_v01.png', dpi=300)
plt.show()
