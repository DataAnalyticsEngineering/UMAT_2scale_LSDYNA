...
extern "C" void umat_cpp_mechanical_(const double& elastic_flag,
                                     const double& mat_id,
                                     const double& yield_shift,
                                     ...
                                     double& hardening_var,
                                     const double& ref_temp)
{
    double temperature = limit_temperature_2minmax(temper);

    auto mat = material(material_id(int(mat_id))); // cm is defined in Fortran as real*8
    ...
    if (int(elastic_flag) == 1) // plastic material model with linear isotropic hardening and no viscous effects
    {
        ...
        if (norm_dev > 0.)
        {
            ...
            if (trial_yield_func > 0.)
            {
                ...
                // assert(eig_val(1).real() > 0.0);
                // assert(eig_val(2).real() > 0.0);
                // assert(eig_val(3).real() > 0.0);
                // assert(eig_val(4).real() > 0.0);
                // assert(eig_val(5).real() > 0.0);
            }
        }
    }

    // Mandel -> Voigt notation
    stress_mandel(seq(3, 5)) /= sqrt(2.);
    stiffness(seq(3, 5), seq(3, 5)) /= 2.;
    stiffness(seq(0, 2), seq(3, 5)) /= sqrt(2.);
    stiffness(seq(3, 5), seq(0, 2)) /= sqrt(2.);
...
{
    auto mat = material(material_id(int(mat_id)));
